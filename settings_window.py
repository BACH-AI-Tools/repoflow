...
class SettingsWindow:
    ...
    def create_widgets(self):
        ...
        # ===== 6. 即梦 MCP 配置 ===== 改为JSON方式
        jimeng_frame = tk.LabelFrame(scrollable_frame, text="  🎨 即梦 MCP 配置 (Logo 生成)  ",
                                     bg='#FFFFFF', fg='#1D1D1F',
                                     font=('SF Pro Display', 13, 'bold') if sys.platform == 'darwin' else ('微软雅黑', 13, 'bold'),
                                     bd=1, relief='solid', padx=20, pady=15)
        ...
        enable_check = tk.Checkbutton(jimeng_frame, text="启用即梦 AI Logo 生成",
        ...
        tk.Label(jimeng_frame, text="MCP 服务器配置 (JSON):", bg='#FFFFFF', fg='#1D1D1F',
                font=('SF Pro Text', 11) if sys.platform == 'darwin' else ('微软雅黑', 11)).grid(row=1, column=0, sticky=tk.NW, pady=8)
        
        # JSON 输入框 - 多行文本
        json_frame = tk.Frame(jimeng_frame, bg='#FFFFFF')
        json_frame.grid(row=1, column=1, sticky=tk.EW, padx=8, pady=8)
        
        from tkinter import scrolledtext
        self.jimeng_json_text = scrolledtext.ScrolledText(
            json_frame,
            height=6,
            width=50,
        ...
        self.jimeng_json_text.pack(fill=tk.BOTH, expand=True)
        
        # 提示文字
        hint_label = tk.Label(
            jimeng_frame,
            text='格式: {"mcpServers": {"服务器名": {"url": "...", "headers": {"emcp-key": "...", "emcp-usercode": "..."}, "type": "sse"}}}',
            bg='#FFFFFF',
            fg='#86868B',
        ...
        hint_label.grid(row=2, column=1, sticky=tk.W, padx=8, pady=(0, 8))
    ...
    def load_config(self):
        ...
        if mcp_json:
            self.jimeng_json_text.delete("1.0", tk.END)
            self.jimeng_json_text.insert("1.0", json.dumps(mcp_json, indent=2, ensure_ascii=False))
        else:
            # 显示正确的示例格置
            example = {
                "mcpServers": {
                    "jimeng": {
                        ...
                        "headers": {
                            "emcp-key": "your_key_here",
                            "emcp-usercode": "your_usercode_here"
                        },
                        "type": "sse"
                    }
                }
            ...
            self.jimeng_json_text.delete("1.0", tk.END)
            self.jimeng_json_text.insert("1.0", json.dumps(example, indent=2, ensure_ascii=False))
    ...
    def save_config(self):
        ...
        import json
        try:
            json_text = self.jimeng_json_text.get("1.0", tk.END).strip()
            if json_text:
                mcp_json = json.loads(json_text)
                config["jimeng"]["mcp_json"] = mcp_json
            else:
                config["jimeng"]["mcp_json"] = {}
        except json.JSONDecodeError as e:
            messagebox.showerror("错误", f"MCP JSON 格式错误:\n{str(e)}", parent=self.window)
            return
